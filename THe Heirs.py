class heirs():
    ...
    def calculate_The_person_share_of_the_heirs(self):
        if "0" in family or "1" in family or "2" in family or "3" in family or "4" in family or "5" in family or "6" in family or "7" in family \
                ...
                or "=" in family or "~" in family or "`" in family or "" in family or money_account[10] >4 or money_account[10] <0:
            Lie_detector[0] = 0
            print("PLEASE WRITE THE CORRECT INFORMATION")

        else:
            if "pass" in self.person[1] or "" == self.person[1] or " " == self.person[1] \
                    or "  " == self.person[1] or "   " == self.person[1] or "    " in self.person[1]:
            ...
            if "pass" in self.person[3] or "" == self.person[3] or " " == self.person[3] \
                    or "  " == self.person[3] or "   " == self.person[3] or "    " in self.person[3]:
...
family = (print("Write the name of the father. If the father is not existent ,pass :"), input() ,
          print("Write the name of the mother. If the mother is not existent ,pass :"), input())
